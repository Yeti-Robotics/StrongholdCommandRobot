...
public class Processor {
	...
	public static Contour findDesignatedContour() {
		...
		for (int i = 0; i < frameQueue.get(CIRCULAR_FIFO_QUEUE_SIZE - 1).size(); i++) {
			if (i == 0) {
				...
				if (frameQueue.get(CIRCULAR_FIFO_QUEUE_SIZE - 1).get(i).getParam(Param.AREA) > designatedContour
						.getParam(Param.AREA)) {
					designatedContour = frameQueue.get(CIRCULAR_FIFO_QUEUE_SIZE - 1).get(i);
				}
			}
	...
	public static double[] getDistanceAzimuth() {
		...
		distance *= Math.cos(Math.asin(GOAL_CENTER_HEIGHT / distance));
		azimuth = ((centerX * HORIZONTAL_FOV) / IMAGE_WIDTH) - (.5 * HORIZONTAL_FOV);
		double[] distanceAzimuth = {
				distance,
				azimuth
		};
	...
	public static void publishDistanceAzimuth() {
		double[] distanceAzimuth = getDistanceAzimuth();
		
		if (distanceAzimuth != null) {
			SmartDashboard.putNumber(DISTANCE_ID, -1);
			SmartDashboard.putNumber(AZIMUTH_ID, 361);
			return;
		}
		SmartDashboard.putNumber(DISTANCE_ID, distanceAzimuth[0]);
		SmartDashboard.putNumber(AZIMUTH_ID, distanceAzimuth[1]);
	}
	...
	public static ArrayList<Contour> readCurrentContourReport() {
		...
		for (int i = 0; i < table.getNumberArray("", DEFAULT_VALUE_ARRAY).length; i++) {
			contours.add(new Contour(table.getNumberArray("", DEFAULT_VALUE_ARRAY)[i],
					table.getNumberArray("", DEFAULT_VALUE_ARRAY)[i], table.getNumberArray("", DEFAULT_VALUE_ARRAY)[i],
					table.getNumberArray("", DEFAULT_VALUE_ARRAY)[i], table.getNumberArray("", DEFAULT_VALUE_ARRAY)[i],
					table.getNumberArray("", DEFAULT_VALUE_ARRAY)[i]));
		}
		return contours;
	}
}
